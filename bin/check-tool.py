...
parser.add_argument("-j", '--jobs', default=16,
...
nostatic_builds = ['extended', 'quadruple', 'ifort-stdsemantics',
  'ifort-quadruple', 'ifort-noflags', 'nagfor-jenkins']
builds = ['dist'] + [b + '-develop' for b in nostatic_builds]
...
for b in builds:
  if 'dist' in b:
    ...
    this_flag = flag
  cmd = ['build-tool.py', b, '-j' + str(args.jobs), this_flag]
  print 'calling ', cmd
  subprocess.call(cmd)
